...
bool ObstacleCameraPerception::Init(
  ...
  std::string work_root = options.root_dir;
  std::string config_file =
      GetAbsolutePath(options.root_dir, options.conf_file);
  CHECK(GetProtoFromFile(config_file, &perception_param_))
  ...
  CHECK(perception_param_.has_tracker_param()) << "Failed to init tracker.";
  {
    ...
    CHECK(tracker_->Init(tracker_init_options)) << "Failed to init: "
                                                << plugin_param.name();
  }
      ...
      << "Failed to init transformer.";
  {
    ...
    CHECK(transformer_->Init(transformer_init_options)) << "Failed to init: "
                                                        << plugin_param.name();
  }
  ...
  if (!perception_param_.has_feature_param()) {
    ...
    CHECK(extractor_->Init(init_options)) << "Failed to init: "
                                          << plugin_param.name();
  }
...
bool ObstacleCameraPerception::Perception(
  ...
  obstacle_postprocessor_options.do_refinement_with_calibration_service = false;
