...
bool DataProvider::FillImageData(int rows, int cols, const uint8_t *data,
  ...
  if (encoding == "rgb8") {
    ...
    uint8_t rgb_data[3 * rows * cols];
    memset(rgb_data, 0, sizeof(rgb_data));
    Yuyv2rgb(data, rgb_data, rows * cols);
    if (handler_ != nullptr) {
      cudaMemcpy(ori_rgb_->mutable_gpu_data(), rgb_data,
                 ori_rgb_->rows() * ori_rgb_->width_step(), cudaMemcpyDefault);
      success = handler_->Handle(*ori_rgb_, rgb_.get());
    } else {
      cudaMemcpy(rgb_->mutable_gpu_data(), rgb_data,
                 rgb_->rows() * rgb_->width_step(), cudaMemcpyDefault);
      success = true;
    }
    rgb_ready_ = true;
...
void DataProvider::Yuyv2rgb(const uint8_t *YUV, uint8_t *RGB, int NumPixels) {
  int i, j;
  uint8_t y0, y1, u, v;
  uint8_t r, g, b;
...
void DataProvider::YUV2RGB(const uint8_t y, const uint8_t u, const uint8_t v, 
...
uint8_t DataProvider::CLIPVALUE(int val) {
  val = val < 0 ? 0 : val;
