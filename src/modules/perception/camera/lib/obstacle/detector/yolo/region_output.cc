...
void get_objects_gpu(const YoloBlobs &yolo_blobs,
  ...
  for (int i = 0; i < num_candidates_vec.size(); i++){
    ...
    get_kernel(block_size, thread_size, num_candidates_vec[i], 
               loc_data, obj_data, cls_data, ori_data, dim_data,
               ...
               all_scales_num_candidates, stream);
  ...
  for (auto it = indices.begin(); it != indices.end(); ++it) {
    ...
    for (size_t j = 0; j < indice.size(); ++j) {
      ...
      obj->sub_type_probs.assign(
          static_cast<int>(base::ObjectSubType::MAX_OBJECT_TYPE), 0.0f);
      float total = 1e-5;
      for (int k = 0; k < num_classes; ++k) {
