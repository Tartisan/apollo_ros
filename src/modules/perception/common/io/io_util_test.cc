...
TEST(CommonIoTest, read_pose_file) {
  const std::string testdata_folder =
      "/apollo/kernels/perception/testdata/common/io/params/";
  std::string filename;
  Eigen::Affine3d pose;
  int frame_id = 0;
  double time_stamp = 0;
...
TEST(CommonIoTest, GetFileList) {
  std::string path = "/apollo/kernels/perception/testdata/lib/data";
  std::vector<std::string> files;
  EXPECT_TRUE(GetFileList(path, "", &files));
  EXPECT_FALSE(GetFileList("/not_exist_path", "", &files));
  EXPECT_TRUE(GetFileList("/apollo/kernels/perception/testdata/lib/data", "txt",
                          &files));
}
