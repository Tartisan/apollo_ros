...
#include "modules/tools/data_conversion/rosbag_to_record/channel_info.h"
...
int convert_Image(std::shared_ptr<apollo::drivers::Image> proto,
  ...
  proto->set_step(rawdata->step);
  proto->mutable_data()->reserve(rawdata->step * rawdata->height);
  proto->set_data((const void *)&rawdata->data[0],
                  rawdata->step * rawdata->height);
  if (rawdata->encoding != "yuyv" && rawdata->encoding != "rgb8") {
    std::cerr << "Wrong pixel fromat:" << rawdata->encoding
...
int main(int argc, char **argv) {
  ...
  for (const rosbag::MessageInstance m : view) {
    ...
    //   // auto pb_msg =
    //   m.instantiate<apollo::prediction::PredictionObstacles>();
    //   pb_msg->SerializeToString(&serialized_str);
    ...
    //   auto pb_msg =
    //   m.instantiate<apollo::localization::LocalizationEstimate>();
    //   pb_msg->SerializeToString(&serialized_str);
    // } else if (channel_name == "/apollo/perception/traffic_light") {
    //   auto pb_msg =
    //   m.instantiate<apollo::perception::TrafficLightDetection>();
    //   pb_msg->SerializeToString(&serialized_str);
    ...
    //   // auto pb_msg =
    //   m.instantiate<apollo::common::monitor::MonitorMessage>();
    //   pb_msg->SerializeToString(&serialized_str);
    ...
    //   // auto pb_msg =
    //   m.instantiate<apollo::drivers::gnss::EpochObservation>();
    //   pb_msg->SerializeToString(&serialized_str);
    // }
    if (channel_name == "/apollo/sensor/velodyne64/compensator/PointCloud2") {
      ...
      pb_msg->SerializeToString(&serialized_str);
    } else if (channel_name == "/apollo/sensor/camera/obstacle/front_6mm" ||
