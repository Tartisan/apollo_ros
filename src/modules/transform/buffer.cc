...
void Buffer::SubscriptionCallbackImpl(
  ...
  for (int i = 0; i < msg_evt->transforms_size(); i++) {
    try {
      ...
      trans_stamped.header.stamp = static_cast<ros::Time>(
...
void Buffer::TF2MsgToCyber(
  ...
  trans_stamped.mutable_header()->set_timestamp_sec(
      tf2_trans_stamped.header.stamp.toSec());
...
apollo::transform::TransformStamped Buffer::lookupTransform(
  ...
  ros::Time tf2_time(time.ToNanosecond());
...
apollo::transform::TransformStamped Buffer::lookupTransform(
  ...
  ros::Time target_time_r(target_time.ToNanosecond());
  ros::Time source_time_r(source_time.ToNanosecond());
  geometry_msgs::TransformStamped tf2_trans_stamped =
      lookupTransform(target_frame, target_time_r, source_frame,
                      source_time_r, fixed_frame);
  apollo::transform::TransformStamped trans_stamped;
  TF2MsgToCyber(tf2_trans_stamped, trans_stamped);
  return trans_stamped;
}
...
bool Buffer::canTransform(const std::string& target_frame,
  ...
  ros::Time time_r(time.ToNanosecond());
  ...
  while (
      ...
      !canTransform(target_frame, source_frame, time_r, errstr) &&
      !cyber::IsShutdown()) {
    usleep(3000);
  }
  bool retval =
...
bool Buffer::canTransform(const std::string& target_frame,
  ...
  while (cyber::Time::Now().ToNanosecond() < start_time + timeout_ns &&
         !canTransform(target_frame, target_time_r, source_frame,
                       source_time_r,
                       fixed_frame) &&
  ...
  bool retval =
      canTransform(target_frame, target_time_r, source_frame,
                   source_time_r, fixed_frame, errstr);
  return retval;
}
